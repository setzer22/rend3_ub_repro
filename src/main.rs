...
use rend3_routine::{common::PerMaterialArchetypeInterface, depth::{DepthRenderableMaterial, AlphaCutoutSpec}};
...
pub struct TerrainMaterialData {
    color: glam::Vec4,
}
...
impl TerrainMaterial {
    pub fn new(color: glam::Vec4) -> Self {
        Self {
            data: TerrainMaterialData { color },
        }
    }
}

impl rend3::types::Material for TerrainMaterial {
    const TEXTURE_COUNT: u32 = 0;
    const DATA_SIZE: u32 = std::mem::size_of::<TerrainMaterialData>() as u32;

    fn object_key(&self) -> u64 {
        // Object key is unused
        0
    }

    fn to_data(&self, slice: &mut [u8]) {
        slice.copy_from_slice(bytemuck::cast_slice(&[self.data]));
    }
...
impl DepthRenderableMaterial for TerrainMaterial {
    const ALPHA_CUTOUT: Option<AlphaCutoutSpec> = None;
}
...
fn main() {
    ...
    let per_material =
        PerMaterialArchetypeInterface::<TerrainMaterial>::new(&renderer.device, renderer.profile);
    let mut data_core = renderer.data_core.lock();
    data_core
        .material_manager
        .ensure_archetype::<TerrainMaterial>(&renderer.device, renderer.profile);
    let depth_routine = rend3_routine::depth::DepthRoutine::new(
        &renderer,
        &data_core,
        &base_rendergraph.interfaces,
        &per_material,
